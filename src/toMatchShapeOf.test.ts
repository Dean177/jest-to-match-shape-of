...
describe('toMatchShapeOf', () => {
  it('checks that expected has the same shape, with the same keys and types', () => {
    ...
    expect(toMatchShapeOfCompare(simpleObject, expectedSimple).pass).toBe(true)
  ...
  it('can handle nested objects', () => {
    ...
    expect(toMatchShapeOfCompare(nestedObject, expectedNested).pass).toBe(true)
...
describe('toMatchOneOf', () => {
  ...
  it('will navigate into nested objects if all of the examples have the key, even with null values', () => {
  ...
  it('passes when the actual object provides more keys than the expected values', () => {
  ...
  it('will fail for undefined keys if they are present on all expected values', () => {
    const example = { a: 2 }
    const optionOne = { a: 3, b : '3' }
